 ...
 * take a JSON key and an expected type template parameter, and throw exceptions
 * of type JSONFile::TypeException if data is stored in the file at that key
 * that does not match the expected type. If it is absolutely necessary for a
 * key to be able to hold multiple valid types, getProperty and setProperty can
 * read and write values as juce::var objects, which will accept any valid JSON
 * data type.
 * 

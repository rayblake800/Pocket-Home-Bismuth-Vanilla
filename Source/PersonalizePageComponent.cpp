...
void PersonalizePageComponent::updateComboBox() {
    ...
    String background = config->getConfigString(ConfigFile::BACKGROUND);
    bool display = false;
...
bool PersonalizePageComponent::updateJSON() {
    ...
    if (edit_name.isVisible()) {
        ...
        ConfigFile::AppItem newFavorite;
    ...
    if (choose_back.getSelectedId() == 1) {
        config->setConfigString(ConfigFile::BACKGROUND,"4D4D4D");
    ...
    if (choose_back.getSelectedId() == 2) {
        ...
        if (value.length() != 6 || !value.containsOnly("0123456789ABCDEF"))
            ...
            config->setConfigString(ConfigFile::BACKGROUND,value);
            color_b = true;

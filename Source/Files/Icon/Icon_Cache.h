...
public:
    ...
    /**
     * @brief  Checks if this object represents a valid cache file. This ensures
     *         that the cache file exists, is not out of date, and contains icon
     *         data. 
     * 
...
private:
    ...
    /**
     * @brief  Reads a two-byte unsigned integer from an arbitrary offset in the
     *         cache file, after ensuring that the cache file is valid and the 
     *         offset is within the file bounds.
     * 
    ...
    /**
     * @brief  Reads a null-terminated character string from an arbitrary offset
     *         in the cache file, after ensuring that the cache file is valid 
     *         and the offset is within the file bounds.
     * 
     * @param offset  The offset in bytes from the beginning of the cache file.
     * 
     * @return        The String at that offset, or the empty string if the file
     *                is invalid, the offset is out of bounds, or the null 

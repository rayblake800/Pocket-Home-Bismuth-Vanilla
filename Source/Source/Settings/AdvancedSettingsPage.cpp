...
void AdvancedSettingsPage::buttonClicked(Button* button){
	PageStackComponent& pageStack=PocketHomeApplication
	::getInstance()->getMainStack();
  if (button == backButton)
    pageStack.popPage(PageStackComponent::kTransitionTranslateHorizontal);
  else if(button == &addLogin){
    spl->switchToModify();
    pageStack.pushPage(spl, PageStackComponent::kTransitionTranslateHorizontal);
  }
    ...
    pageStack.pushPage(ppc, PageStackComponent::kTransitionTranslateHorizontal);
    ...
    pageStack.pushPage(inputsettings, PageStackComponent::kTransitionTranslateHorizontal);
  }
  else if(button == &dateandtime){
    pageStack.pushPage(datetime, PageStackComponent::kTransitionTranslateHorizontal);
  }
}

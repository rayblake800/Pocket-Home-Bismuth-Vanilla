...
// Extra characters applied when calculating title width, defining title
// padding space relative to the font size.
...
// Finds the width of the background area that will be drawn behind the
// button's title.
...
(juce::Graphics &g, bool isMouseOverButton, bool isButtonDown)
{
    ...
    // Draw title:
    g.setColour(findColour(isSelected()
                ? selectedTextColourId : textColourId));
    g.setFont(titleFont);

...
/**
 * @brief  Creates, contains, and arranges MenuButton components for one folder
 *         displayed using the Paged menu format.
 *
 *  The Paged::FolderComponent divides its menu items over several folder
 * pages, only one of which may be visible at a time. The visible folder page
 * is usually the one containing the selected menu item, but any folder page
 * may be set as visible if no menu item is selected. The Paged::MenuComponent
 * is responsible for positioning the FolderComponent so that its active folder
 * page is visible.
 *
 *  Menu items within each folder page are arranged in a grid. The dimensions
 * of this grid are defined in the apps.json file, accessible through
 * AppMenu::Settings. Paged::FolderComponent provides several functions for
 * finding a menu item's position in its page grid, and for finding the index of
 * a menu item at a specific position on the page. These are supplied so that
 * the Paged::InputHandler can more easily provide navigation controls.
 */
...
public:
     ...
     *
     * @return  The column index of the selected item within its folder page,
     ...
     *
     * @return  The row index of the selected item within its folder page, or
